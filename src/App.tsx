      ...
      {open && (
        <div className="border-t border-ink-line bg-ink-base px-5 py-4 lg:hidden">
          {[...NAV_LINKS, { href: '#/buy', label: 'GET THE BOOK' }].map((link) => (
            <a key={link.href} href={link.href} onClick={() => setOpen(false)} className="block border-b border-ink-line py-4 font-mono text-[13px] uppercase tracking-[0.12em] text-bright hover:text-signal">
              {link.label}
            </a>
          ))}
        </div>
        ...
        <div className="hero-grid mx-auto max-w-7xl px-5 lg:px-8">
          <div className="hero-grid-top">
            ...
            <h1 className="break-words font-heading text-[clamp(3.625rem,8vw,6.75rem)] font-bold leading-[0.94] tracking-[0.015em] text-bright">
              ZERO <span className="animate-flicker bg-gradient-to-r from-[#f2b94f] via-[#d8952f] to-[#b97824] bg-clip-text text-transparent">ACCOUNT</span>
            </h1>
          ...
          <div className="hero-grid-bottom mt-9 flex flex-wrap gap-4 lg:mt-0">
            <a href="#subscribe" onClick={scrollToId('subscribe')} className="glitch-hover group inline-flex min-h-[44px] items-center gap-2 bg-signal px-5 py-3 font-mono text-[12px] font-semibold uppercase tracking-[0.16em] text-ink-base transition-all hover:bg-signal-glow hover:shadow-[0_0_24px_rgba(232,163,61,0.5)] sm:min-h-0">GET CHAPTER ONE FREE <span className="inline-flex"><ArrowRight size={16} className="transition-transform group-hover:translate-x-1" /></span></a>
            <a href="#evidence" onClick={scrollToId('evidence')} className="glitch-hover inline-flex min-h-[44px] items-center gap-2 border-2 border-signal/70 px-5 py-3 font-mono text-[12px] font-semibold uppercase tracking-[0.16em] text-signal transition-all hover:-translate-y-0.5 hover:bg-signal/10 hover:shadow-[0_0_18px_rgba(232,163,61,0.35)] sm:min-h-0">EXPLORE THE CASE</a>
          </div>
        </div>
      </section>
      <Ticker />
      ...
      <main className="mx-auto max-w-4xl px-5 py-12 sm:py-16 lg:px-8 lg:py-24">
        <div className="corner-frame relative border border-ink-muted bg-[#1a1813] p-7 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.8)] sm:p-12">
          ...
          <div className="relative mb-8 flex flex-wrap items-center justify-between gap-4 border-b border-[#3a352a] pb-5">
            <div className="flex items-center gap-3"><FileText size={17} className="text-signal" /><span className="font-mono text-[12px] uppercase tracking-[0.24em] text-bright-muted">CHAPTER ONE // SCANNED_DOCUMENT</span></div>
            <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-bright-faint">EST. READ TIME: 6 MIN</span>
          </div>
...
function BuyPage() {
  return <><PageIntro label="BUY NOW" title="ACQUIRE THE FILE">The truth is expensive. The book costs less.</PageIntro><main className="mx-auto max-w-5xl px-5 py-12 sm:py-16 lg:px-8 lg:py-24"><div className="grid gap-16 lg:grid-cols-[1fr_0.8fr]"><div><CaseLabel className="mb-5 block">RETAIL ACCESS // SELECT CHANNEL</CaseLabel><div className="grid gap-4 sm:grid-cols-2">{RETAILERS.map((retailer) => <a key={retailer} href="#" data-todo="insert real purchase link" className="corner-frame group flex items-center justify-between border border-signal/60 px-5 py-4 font-mono text-[12px] font-semibold uppercase tracking-[0.16em] text-signal transition-all hover:-translate-y-0.5 hover:bg-signal hover:text-ink-base hover:shadow-[0_14px_30px_-16px_rgba(232,163,61,0.45)]">{retailer}<span className="inline-flex"><ArrowRight size={15} /></span></a>)}</div><p className="mt-6 font-mono text-[11px] uppercase tracking-[0.2em] text-bright-faint">// RELEASE_WINDOW: MID-SEPTEMBER 2026 — LINKS ACTIVATE AT LAUNCH</p></div><div><CaseLabel className="mb-5 block">&gt; REQUEST_CLEARANCE ::</CaseLabel><h2 className="font-heading2 text-2xl font-bold tracking-wide text-bright">GET THE SAMPLE CHAPTER FIRST</h2><p className="mt-3 mb-7 font-mono text-[12px] uppercase tracking-[0.16em] text-bright-muted">// OPENING TRANSMISSION SENT INSTANTLY TO YOUR INBOX</p><ClearanceRequest /></div></div></main></>;
}
